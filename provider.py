...
``OXYLABS_API_URL``; auth header ``x-api-key``)::
...
_API_KEY_ENV = "OXYLABS_API_KEY"
_API_URL_ENV = "OXYLABS_API_URL"
...
def _read_api_key() -> str:
    ...
    api_key = _env_value(_API_KEY_ENV)
    if not api_key:
        raise ValueError(
            f"{_API_KEY_ENV} is not configured for Hermes. Add it to your "
            f"shell environment or ~/.hermes/.env, then start a new Hermes "
            f"session or restart the Hermes gateway. Get your API key at "
            f"{_API_KEY_URL}"
        )
    return api_key
...
def _base_url() -> str:
    ...
    return (_env_value(_API_URL_ENV) or _DEFAULT_BASE_URL).rstrip("/")


def _env_value(name: str) -> str:
    """Resolve a Hermes credential/config env value."""
    try:
        ...
        if exc.__class__.__name__ == "UnscopedSecretError":
            logger.debug("Oxylabs env %s unavailable outside a Hermes secret scope", name)
            return ""
        logger.debug("Could not read Hermes secret %s: %s", name, exc)
    ...
    try:
        ...
        logger.debug("Could not read Hermes env %s: %s", name, exc)

    return os.getenv(name, "").strip()


def _is_explicitly_configured() -> bool:
    """Return True when Hermes config explicitly selects this backend."""
    try:
        from hermes_cli.config import load_config

        web_config = load_config().get("web", {})
    except Exception as exc:  # noqa: BLE001
        logger.debug("Could not read Hermes web config: %s", exc)
        return False
    ...
    return any(
        str(web_config.get(key) or "").strip().lower() == "oxylabs"
...
class OxylabsWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        """Return True when configured enough for Hermes to select us.

        If the user explicitly selected ``oxylabs`` in ``web.*_backend``, stay
        available even without a key so tool calls surface this plugin's
        actionable setup error instead of silently falling back to another
        backend.
        """
        return bool(_env_value(_API_KEY_ENV)) or _is_explicitly_configured()
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``OXYLABS_API_KEY``) propagate to the dispatcher's top-level
